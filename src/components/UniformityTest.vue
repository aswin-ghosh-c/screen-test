<template>
  <div class="d-flex flex-column align-items-center justify-content-center w-100 h-100" style="width:80vw;min-height:100vh;">
    <div class="text-center mb-3">
      <i class="bi bi-grid-3x3-gap-fill text-warning mb-2" style="font-size:2.2rem;"></i>
      <h2 class="mb-1 fw-bold title-gradient" style="font-size:1.5rem; letter-spacing:0.5px;">{{ $t('test.uniformity') }}</h2>
      <p class="mb-2 text-secondary" style="font-size:1.08rem;">{{ $t('uniformity.instructions') }}</p>
    </div>
    <div class="w-100 d-flex flex-wrap justify-content-center align-items-center" style="gap:8px;">
      <div v-for="i in 9" :key="i" class="border border-2 rounded" style="width:90px;height:90px;background:#fff;"></div>
    </div>
